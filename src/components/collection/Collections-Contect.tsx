...
function CollectionsContect() {


    const [filterName, setFilterName] = useState<string>("all")
    const fliteringProduct = (filterName === "all" || filterName == null) ? products : products.filter(ele => ele.category === filterName)

    const handleFiltersClick = (ele: Filter) => {
        setFilterName(ele.word)
        redirect(`/collections?filter=${ele.word}`)
    }
                        ...
                        return (
                            <div
                                ...
                                className={`capitalize h-1/2 w-fit p-5 flex justify-center items-center rounded-2xl border cursor-pointer 
                                            ${filterName === ele.word ? "bg-white text-black" : "border-white/10 hover:bg-white/9 bg-white/5"}
                                          `}
                            >
                                {ele.word}
                            </div>
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 h-fit mb-10">
                {
                    fliteringProduct.map(ele => {
                        return (
                            <div className="group text-white" key={ele.id} onClick={() => { }}>
                                <div className="bg-zinc-900 group-hover:bg-zinc-800 w-full h-100 rounded-xl flex flex-col space-y-5 overflow-hidden cursor-pointer" >
                                    ...
                                    <div className="h-full flex flex-col capitalize w-10/12 mx-auto space-y-4">
                                        ...
                                        <div className=" flex justify-between w-full">
                                            ...
                                            <Button variant={"ghost"} className="rounded-full cursor-pointer" onClick={() => { }}>
                                                <Info />
                                            </Button>
                                        </div>
                                    </div>
